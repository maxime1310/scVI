...
class Stats:
    ...
    def callback(self, model, *data_loaders, classifier=None, mode=None):
        if self.epoch == 0 or self.epoch == self.n_epochs or (
            ...
            for data_loader, name in zip(data_loaders, self.names):
                self.add_ll(model, data_loader, name=name, mode = mode)
                self.add_accuracy(model, data_loader, classifier=classifier, name=name)
            self.save_best_params(model)
        self.epoch += 1

    def add_ll(self, model, data_loader, name='train', mode=None):
        models = [VAEF]
        if type(model) in models:
            log_likelihood = compute_log_likelihood(model, data_loader, mode)
